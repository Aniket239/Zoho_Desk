      ...
      if request_data.dig('_json', 0, 'eventType') == 'Ticket_Update'
        ...
        if response.code == 200 && recipient_email
          ...
          unless customer_care_emails.include?(normalized_email)
            ...
            button_regex = /<a href="https:\/\/0835-49-37-10-9.ngrok-free.app\/tickets\/issue[^"]*"[^>]*>\s*Issue Solved\s*<\/a>/
            prompt_regex = /<h3>!!!\s*Kindly provide solution for this issue via replying to this mail. If the issue is solved from your end then kindly click\s*. Otherwise, the issue will remain marked as open in our system.\s*!!!<\/h3>/i
            cleaned_content = contents[0].to_s.gsub(button_regex, '').gsub(prompt_regex, '')
            if note
              content = <<~HTML
              <h3>!!! Kindly provide solution for this issue via replying to this mail. If the issue is solved from your end then kindly click <a href="https://0835-49-37-10-9.ngrok-free.app/tickets/issue?ticketId=#{ticket_id}&agent_id=#{agent_id}&assignee_name=#{recipient_name}" style="background-color: #4CAF50; border-radius: 5px; color: white; padding: 5px 10px 4px 10px; font-size: 14px; font-family: Helvetica, Arial, sans-serif; text-decoration: none; display: inline-block;">Issue Solved</a>. Otherwise, the issue will remain marked as open in our system. !!!</h3>
