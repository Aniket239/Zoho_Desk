    ...
    def process_ticket_update(event)
      ...
      if response.code == 200
        ...
        if access_token
          ...
          p "=========================================== thread id ========================================"
          p threads_response_content = threads_response.parsed_response
          thread_id = threads_response_content["data"][0]["id"]
          threads_content = HTTParty.get("https://desk.zoho.in/api/v1/tickets/#{ticket_id}/threads/#{thread_id}/originalContent",headers: { 'Authorization' => "Zoho-oauthtoken #{access_token}" })
          p "=========================================== thread id ========================================"
          forward_response = HTTParty.post("https://desk.zoho.in/api/v1/tickets/#{ticket_id}/sendReply", body: {
            channel: "EMAIL",
            isForward: 'true',
            fromEmailAddress: "support@thejaingroup.zohodesk.in",
            to: 'system4@thejaingroup.com',  # The recipient's email address
            content: threads_content["content"],
            # ,142998000000299471
            contentType: 'html'  # or 'plainText' if applicable
          }.to_json, headers: {
            "Authorization" => "Zoho-oauthtoken #{access_token}",
          })
      
          if forward_response.code == 200
            p "Ticket forwarded successfully"
          else
            p "Failed to forward ticket"
            p forward_response.parsed_response  # To understand what went wrong
          end
        end
      else
